function __print_nightshadeos_functions_help() {
cat <<EOF
Additional NightShadeOS functions:
- cout:               Changes directory to out.
- mmp:                Builds all of the modules in the current directory and pushes them to the device.
- mmap:               Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:               Builds all of the modules in the supplied directories and pushes them to the device.
- nightshadegerrit:   A Git wrapper that fetches/pushes patch from/to NightShadeOS Gerrit Review.
- nightshaderebase:   Rebase a Gerrit change and push it again.
- nightshaderemote:   Add git remote for NightShadeOS Gerrit Review.
- aospremote:         Add git remote for matching AOSP repository.
- cafremote:          Add git remote for matching CodeAurora repository.
- githubremote:       Add git remote for NightShadeOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the NightShadeOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch nightshade_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/nightshade-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.nightshade.device | grep -q "$NightShade_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $NightShade_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function nightshaderemote()
{
    ...
    git remote rm nightshade 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    local NightShade="true"
    if [ -z "$REMOTE" ]
    then
    ...
    then
        ...
        local PFX="NightShadeOS/"
    else
        local PROJECT=$REMOTE
    fi

    local NIGHTSHADE_USER=$(git config --get review.review.nightshade.org.username)
    if [ -z "$NIGHTSHADE_USER" ]
    then
        git remote add nightshade ssh://review.nightshadeos.org:29418/$PFX$PROJECT
    else
        git remote add NightShade ssh://$NIGHTSHADE_USER@review.nightshadeos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'nightshade' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/NightShadeOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.nightshade.device | grep -q "$NIGHTSHADE_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        nightshaderemote
        git push nightshade HEAD:refs/heads/'$1'
    fi
    '
}

function nightshadegerrit() {
    ...
    local user=`git config --get review.review.nightshadeos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "nightshadegerrit" ]; then
...
function nightshaderebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "NightShadeOS Gerrit Rebase Usage: "
        echo "      nightshaderebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.nightshadeos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.nightshade.device | grep -q "$NIGHTSHADE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/nightshade/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $NIGHTSHADE_FIXUP_COMMON_OUT ]; then
